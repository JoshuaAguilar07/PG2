...
<div class="max-w-2xl mx-auto bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-lg mt-10">
    <h1 class="text-2xl font-bold text-gray-800 dark:text-gray-100 mb-6">Editar Unidad de Medida</h1>
    ...
    <form action="{{ route('unidades.update', $unidad) }}" method="POST" class="space-y-6">
        ...
        <div>
            ...
            <input type="text" name="nombre" id="nombre" value="{{ old('nombre', $unidad->nombre) }}"
        ...
        <div>
            ...
            <input type="text" name="abreviatura" id="abreviatura" value="{{ old('abreviatura', $unidad->abreviatura) }}"
                   class="mt-1 block w-full rounded-lg border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100 focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
            @error('abreviatura')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
            @enderror
        ...
        <div class="flex items-center">
            <input type="checkbox" name="activa" id="activa" value="1" {{ old('activa', $unidad->activa) ? 'checked' : '' }}
