...
<div class="max-w-2xl mx-auto bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-lg mt-10">
    <h1 class="text-2xl font-bold text-gray-800 dark:text-gray-100 mb-6">Nuevo Estado de Insumo</h1>
    ...
    <form action="{{ route('estados.store') }}" method="POST" class="space-y-6">
        ...
        <div>
            ...
            <textarea name="descripcion" id="descripcion" rows="3"
                      class="mt-1 block w-full rounded-lg border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100 focus:ring-blue-500 focus:border-blue-500 sm:text-sm">{{ old('descripcion') }}</textarea>
            @error('descripcion')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
            @enderror
        </div>
