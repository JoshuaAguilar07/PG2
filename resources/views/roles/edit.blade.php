...
<div class="flex justify-center items-center mt-10">
    <div class="w-full max-w-lg bg-white dark:bg-gray-800 p-8 rounded-2xl shadow-lg">
        <h1 class="text-2xl font-bold text-gray-800 dark:text-gray-100 mb-6">Editar Rol</h1>
        ...
        <form action="{{ route('roles.update', $role) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <textarea name="descripcion" id="descripcion" rows="3"
                          class="mt-1 block w-full rounded-lg border-gray-300 dark:border-gray-700 shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm dark:bg-gray-900 dark:text-gray-100">{{ old('descripcion', $role->descripcion) }}</textarea>
                @error('descripcion')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
